...
df_old_sample=df_old.sample(frac=0.001,random_state=23)
...
X_train_old, X_test_old, y_train_old, y_test_old  = train_test_split(data, target, test_size=0.2 ,random_state=23)

std_scaler = preprocessing.StandardScaler().fit(X_train_old)
X_train_old_scaled = std_scaler.transform(X_train_old)
X_test_old_scaled = std_scaler.transform(X_test_old)
...
svc_grid.fit(X_train_old_scaled, y_train_old)
...
y_svc = svc.predict(X_test_old_scaled)
...
result_metrics = store_metrics(model=svc, model_name='Support Vector Machine',
                               y_test=y_test_old, y_pred=y_svc,
...
# +
# Fitting the grid search to find the best parameter combination
LR_grid.fit(X_train_old_scaled, y_train_old)
...
y_LR = LR.predict(X_test_old_scaled)
...
df_new_sample=df_new.sample(frac=0.007,random_state=23)
...
X_train_new, X_test_new, y_train_new, y_test_new  = train_test_split(data, target, test_size=0.2 ,random_state=23)

std_scaler = preprocessing.StandardScaler().fit(X_train_new)
X_train_new_scaled = std_scaler.transform(X_train_new)
X_test_new_scaled = std_scaler.transform(X_test_new)

# +
# Fitting the grid search to find the best parameter combination
svc_grid.fit(X_train_new_scaled, y_train_new)
...
# Predict target variable for the test set
svc = svc_grid.best_estimator_
y_svc = svc.predict(X_test_new_scaled)
# -

result_metrics = store_metrics(model=svc, model_name='Support Vector Machine',
                               y_test=y_test_new, y_pred=y_svc,
                               result_df=result_metrics)
# Show the interim result                               
result_metrics
...
# +
LR_grid.fit(X_train_new_scaled, y_train_new)
...
# Predict target variable for the test set
LR = LR_grid.best_estimator_
y_LR = LR.predict(X_test_new_scaled)
# Calculate the metrics for the optimal LR model and store them in the result_metrics DataFrame 
# The model will be stored as well in the DataFrame
result_metrics = store_metrics(model=LR, model_name='Logistic Regression',
                               y_test=y_test_new, y_pred=y_LR,
                               result_df=result_metrics)
# Show the interim result                               
result_metrics
# -
